...
class VerifyEmailNotification extends VerifyEmail
{
    ...
    private function buildMail($notifiable): MailMessage
    {
        $mail = (new MailMessage)
            ...
            ->greeting(__('notification.greetings', ['email' => $notifiable->email]));

        return match ($this->actionRoute) {
            'v1.verification.verify' => $this->getEmailVerifyMailMessage($mail, $notifiable)
                ->line(__('notification.email.expired_at.text', ['value' => config('auth.verification.expire') / 60])),
    ...
    {
        return $mail
            ->line(__('notification.email.verify.context'))
            ->action(
                text: __('notification.email.verify.action-text'),
    ...
    {
        return $mail
            ->line(__('notification.email.password-reset.context'))
            ->action(
                text: __('notification.email.password-reset.action-text'),
                url: $this->verificationEmailUrl(
                    $notifiable,
